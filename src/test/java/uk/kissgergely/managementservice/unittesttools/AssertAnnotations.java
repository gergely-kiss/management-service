...
public class AssertAnnotations {
	...
	private static void assertAnnotations(List<?> annotationClasses, List<Annotation> annotations) {
		...
		annotationClasses.forEach(ac -> {
			long cnt = annotations.stream().filter(a -> a.annotationType().isAssignableFrom((Class<?>) ac)).count();
			if (cnt == 0) {
				throw new AssertionError(String.format("No annotation of type %s found", ((Class<?>) ac).getName()));
			}
		});
	}
